...
// Used to send rgb color data to ColorPickerViewController
protocol ReceiveHueColorDelegate{
...
class ColorPicker: UIView {
    ...
    // ------------------------------------------------------------------------------------------------------------------------------
    var receiveHueColorDelegate: ReceiveHueColorDelegate?
    ...
    // MARK: - Override methods
    // ------------------------------------------------------------------------------------------------------------------------------
    func setReceiveHueColorDelegate(incomingDelegate: ReceiveHueColorDelegate){
        receiveHueColorDelegate = incomingDelegate
    }
    ...
    private func handleTouch(_ touch:UITouch, finished:Bool) {
        ...
        currentPoint = CGPoint(x: max(0, min(bounds.width, point.x)) / bounds.width , y: max(0, min(bounds.height, point.y)) / bounds.height)
        _h = currentPoint.x
        
        receiveHueColorDelegate?.receiveHueColor(hue: _h)
    ...
    func drawHueDragHandler(frame: CGRect) {
        ...
        polygonPath.move(to: CGPoint(x: frame.minX + 4, y: frame.maxY - 10))
        ...
        polygon2Path.move(to: CGPoint(x: frame.minX + 4, y: frame.minY + 10))
        ...
        drawLineFromPoint(start: CGPoint(x: frame.minX + 4, y: frame.minY), toPoint: CGPoint(x: frame.minX + 4, y: frame.maxY), ofColor: UIColor.white, inView: self)
    }
